...
//    @Override
    public int compareTo(CombinationKey combinationKey) {
        ...
        if(this.firstKey.compareTo(combinationKey.getFirstKey())!=0)
            return Integer.parseInt(this.firstKey.toString().substring(4,5))<Integer.parseInt(combinationKey
                    .getFirstKey().toString
                    ().substring(4,5))?-1:1;
        else if (this.secondKey.compareTo(combinationKey.getSecondKey())!=0)
        {
            return Integer.parseInt(this.secondKey.toString())<Integer.parseInt(combinationKey.getSecondKey()
                    .toString())?-1:1;
        }
        else
        {
